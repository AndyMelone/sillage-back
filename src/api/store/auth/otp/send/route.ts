 ...
 * Response : { message }
 */
export const POST = async (req: MedusaRequest<SendOtpBody>, res: MedusaResponse) => {
  ...
  if (!pin || typeof pin !== "string") {
    return res.status(400).json({ error: "Le code de sécurité (PIN) est requis." })
  }
  ...
  try {
    await sendOtpWorkflow(req.scope).run({
      input: { phone: normalizedPhone, pin, channel },
    })
